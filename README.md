...
cp .env.example .env              # OPENAI_API_KEY 입력 (없으면 룰 baseline으로 동작)
```

### 크롤링
```bash
# 사용법: npm run crawl <스토어URL> <상품수limit> [ocr]
...
```jsonc
{
  ...
  "provenance": {                     // ← 각 값을 "어떻게 얻었는지" (정직성)
    ...
    "lowest_price": { "method": "empty", "reason": "가산점 미구현. 매칭키 syncNvMid 확보 → 확장 가능" }
  },
...
| `lowest_price` | **공란(가산점)** | 매칭키 `syncNvMid` 확보, 실조회는 미구현 |
...
**해결 못한 것 / 더 시간이 있었다면**: `lowest_price` 실조회(매칭키는 확보), 3축 옵션의 완전한 표현, OCR 속도 최적화(멀티이미지 단일호출), 증분 재크롤.

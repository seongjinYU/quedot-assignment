...
> **OCR은 자동입니다.** 근거가 부족한 상품(셀러태그·본문 텍스트 없음 + 상세이미지 있음)에만 알아서 발화합니다(자가복구와 같은 안전망). 네이버는 셀러태그가 있어 자동 스킵, 고도몰의 이미지전용 상세에만 동작 — `OPENAI_API_KEY`가 있으면 켜지고, 끄려면 `noocr`.
...
> **트레이드오프**
>
> 설명 근거가 아예 없는 상품은 USP를 **억지로 지어내지 않고 비웠습니다**(지어내는 것보다 비우는 게 낫다는 원칙). 다만 글이 아닌 이미지라, OCR 정확도에 기대야 하는 한계는 남습니다.
...
- **어댑터 깨짐 모니터링(품질 SLA)** — 사이트 구조가 바뀌면 어댑터가 **조용히 빈 값**을 뱉을 수 있습니다. 수집량·필드 채움률이 급감하면 경보를 울리고, 스토어별 골든 샘플로 회귀를 잡습니다(지금의 품질 리포트를 모니터링 지표로 승격).

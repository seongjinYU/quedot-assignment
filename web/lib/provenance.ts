...
// 신뢰(결정적)는 무채색, 의심(AI·자가복구·공란)은 유채색으로 시선을 끈다.
// Tailwind v4가 소스에서 클래스 문자열을 스캔하므로 반드시 "리터럴 전체 문자열"로 둔다.
...
// 다축 옵션을 큐닷 option1/2 2칸에 배치하는 의미 판단만 AI가 한 것.
// 값 자체는 원본(결정적)이므로 일반 AI(생성)와 색·라벨을 분리해 과장 표시를 막는다.
export const OPTION_FIELDS = new Set<ProductField>(['option1', 'option2']);

export const PLACEMENT_STYLE: MethodStyle = {
  label: 'AI 배치',
  desc: '값은 원본(결정적) · 다축을 option1/2 2칸에 넣는 배치만 AI',
  dot: 'bg-teal-500',
  tint: 'bg-teal-50',
  badge: 'bg-teal-50 text-teal-700 ring-teal-200',
};

/** 필드+provenance 로 실제 표시 스타일을 결정. 옵션의 ai 는 "AI 배치"로 분리. */

...
- [x] **lowest_price 정확도 버그픽스 추가 2종** ✅ (실데이터 관찰로 발견)
  - ④ 옵션별 가격 무시: 상품단위 1회 조회한 최저가를 전 SKU에 일괄복사 → 옵션마다 판매가 다른 택1 세트(에피베리어 98,300 vs 슬립밸런스 119,300)에 다른 옵션값 새어 "판매가<최저가" 모순. **SKU별 적용**(자기 판매가가 시장최저보다 싸면 판매처 최저) + validate `lowest_above_sale` 안전망 가드
  - ⑤ 복합세트 오매칭: LLM이 "비슷한 케피 세트"를 동일상품이라 억지판정(목욕스펀지세트 → 슬라임세트 32,800·3종세트 23,320 오매칭). 프롬프트에 "복합 번들은 구성품 똑같이 안 갖추면 제외, 없으면 빈 배열" 명시 → 복합세트 null(비교불가) 정직 처리. 검증: 복합세트 3회 모두 [] / 단순상품(누들레이저→옥션) 인정 유지
  - ⚠️ ~~자사 카탈로그 우선권~~ → **해결(방안 A)**: 아래 참조
- [x] **lowest_price 자사몰 제외 (방안 A)** ✅ 근본문제 해결 — syncNvMid가 자사 스마트스토어를 가리켜 "최저가=자사 판매가(=sales_price 복사)"라 의미가 없던 문제.
  - 자사 식별: `pid==mid` OR 네이버 스마트/브랜드스토어 링크 → `isSelf` 마킹. lowest_price 후보에서 **타몰(non-self)만** 사용(자사 가격은 이미 sales_price에 있음).
  - lowest_price 정의를 "우리 외 다른 곳(타몰)의 최저가"로 명확화. 타몰 최저 vs 판매가 SKU별 비교는 유지.
  - 타몰에 동일품 없으면 → `null + "타몰 동일상품 없음(자사몰 독점 추정)"` (자사 가격 억지로 안 넣음).
  - 검증: 누들레이저 1+1 → 자사(제로파운더스 18,900) 제외하고 타몰(옥션 18,890/G마켓) 채택 / 해피세트 복합 → null(자사독점). tsc + LLM 일관성 4/4.
